...
const Login = () => {
  const { handleLogin } = useAuth(); // Access the handleLogin mutation
  ...
  const onSubmit = (data) => {
    handleLogin.mutate(data, {
      onSuccess: () => {
        ...
        router.push("/spaces"); // Redirect to /spaces on successful login
      },
      onError: (error) => {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <button
            ...
            className={`w-full px-4 py-2 text-white font-medium rounded-md ${
              handleLogin.isPending ? "bg-cyan-300" : "bg-cyan-500 hover:bg-cyan-600"
